...
class Service:
    ...
    def stream(self, inputstream, jobname):
        ...
        is_valid, lines = self.validator.validate(streamcopy)
        if is_valid:
            if self.job_queue.is_dirty():
                logging.error("[ hc ] job queue dirty. fix long gcode lines, reset, resubmit")
            else:
            ...
            self.job_queue.dirty(True)
            logging.error("[ hc ] queued job contains gcode lines that exceed " + str(self.validator.rx_buffer_size) + " bytes")

            # Log each invalid line separately
            for line_num, line_content, line_size in lines:
                logging.error("[ hc ] line " + str(line_num) + " exceeds buffer size: '" + line_content + "' (" + str(line_size) + " bytes)")
            logging.error("[ hc ] job queue dirty. fix long gcode lines, reset, resubmit")
